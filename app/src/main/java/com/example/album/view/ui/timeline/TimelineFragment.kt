...
import com.example.album.model.data.adapters.ImageAdapter
...
class TimelineFragment : Fragment() {
    ...
    private fun getTimelineData() {
        ...
        viewModel.getTimelineData(user).observe(activity!!, Observer {
            shimmerFrameLayout.visibility = View.GONE
            shimmerFrameLayout.stopShimmer()
            var manager = GridLayoutManager(context, 4)
            recyclerView.layoutManager = manager
            var adapter = ImageAdapter(it, activity!!.supportFragmentManager)
