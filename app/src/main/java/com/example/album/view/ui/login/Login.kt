...
class Login : Fragment(), View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        if(v?.id ==R.id.btn_sign_in)
        {
            ...
            loginViewModel.login(edt_email.text.toString(),edt_password.text.toString(),this.context!!).observe(activity!!
            , Observer {
                    if(it)
                    {
...
*/
        ...
        {
            manager.beginTransaction().replace(R.id.main_screen,
                Signup.newInstance()
            ).commit()
        }
    }
